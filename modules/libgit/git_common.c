...
int optional_str_arg(const char **out, struct args_info *args, const char *opt,
        const char *def) {
...
int match_uint16_arg(uint16_t *out, struct args_info *args, const char *opt) {
...
int match_uint32_arg(uint32_t *out, struct args_info *args, const char *opt) {
    const char *found = match_numeric_arg(args, opt);
    uint16_t val;
    char *endptr = NULL;
...
static int match_int_internal(int *out, const char *str, int allow_negative,
        const char *opt) {
    char *endptr = NULL;
...
int match_int_arg(int *out, struct args_info *args, const char *opt,
        int allow_negative) {
    const char *found = match_numeric_arg(args, opt);
    if (!found)
        return 0;
...
void treeish_to_tree(git_tree **out, git_repository *repo,
        const char *treeish) {
    git_object *obj = NULL;
...
int resolve_refish(git_annotated_commit **commit, git_repository *repo,
        const char *refish) {
    git_reference *ref;
    git_object *obj;
    int err = 0;
...
int index_matched_path_cb(const char *path, const char *matched_pathspec, void *payload)
{
    ...
    if (matched_pathspec)
    {
        printf("matched_pathspec: %s", matched_pathspec);
    }
    /*
...
int git_add(const char* path, char* file) {
    ...
    if (error < 0)
    {
        ...
        printf("Error: %d and %s", e->klass, e->message);
        goto SHUTDOWN;
    }
    ...
    {
        const git_error *e = giterr_last();
        printf("Error: %d, %d, %s", error, e->klass, e->message);
    }
    else
    {
    ...
    error = git_index_add_all(index, &paths, GIT_INDEX_ADD_DEFAULT, index_matched_path_cb, NULL);
    if (error < 0)
    {
        const git_error *e = giterr_last();
        printf("Error: %d, %d, %s", error, e->klass, e->message);
