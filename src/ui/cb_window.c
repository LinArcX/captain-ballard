...
GtkWidget* create_filechooser_dialog(char *init_path, GtkFileChooserAction action)
{
    ...
    switch (action) {
        ...
        case GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER:
            wdg = gtk_file_chooser_dialog_new("Select folder", NULL, action,
                    "Cancel", GTK_RESPONSE_CANCEL,
                    "Open", GTK_RESPONSE_OK,
                    NULL);
            break;
...
void add_new_path(GtkWidget *widget, gpointer data){
    ...
    wdg = create_filechooser_dialog(dir_name, GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER);
...
{
    GtkWidget *window, *view, *scrolled_win, *hbox, *vbox, *btn_add, *btn_remove, *btn_close;
...
int show_launcher_window(char *full_address)
{
    ...
    status = g_application_run(G_APPLICATION(app), 0, &full_address);
    g_object_unref (app);
