...
int show_nuklear_window(char ***all_files) {
    ...
    while (!glfwWindowShouldClose(win)) {
        ...
        if (nk_begin(ctx, "Show", nk_rect(0, 0, WINDOW_WIDTH, WINDOW_HEIGHT),
            ...
            if (all_files) {
                for (size_t i = 0; i < vector_size(all_files); i++) {
                    if (nk_tree_push_id(ctx, NK_TREE_TAB, all_files[i][0], NK_MINIMIZED, i)) {
...
//                // if (nk_chart_begin_colored(ctx, NK_CHART_LINES, nk_rgb(255,
//                0,
//                // 0),
//                //            nk_rgb(150, 0, 0), 32, 0.0f, 1.0f)) {
//                //    nk_chart_add_slot_colored(ctx, NK_CHART_LINES, nk_rgb(0,
//                0,
//                //    255),
